...
/**
 * WP_List_Table subclass for the Capstone Submissions admin screen.
 */
class TL_Capstone_Submission_List_Table extends WP_List_Table {
	...
	public function __construct( TL_Capstone_Submission_Repository $repo ) {
		parent::__construct(
			array(
				'singular' => 'capstone_submission',
				'plural'   => 'capstone_submissions',
				'ajax'     => false,
			)
		);
		$this->repo = $repo;
	...
	protected function column_student( $item ) {
		...
		$detail_url = add_query_arg(
			array(
				'page' => 'curriki-learn-capstone-submissions',
				'view' => 'detail',
				'id'   => absint( $item->id ),
			),
			admin_url( 'admin.php' )
	...
	public function prepare_items() {
		...
		$per_page     = $this->get_items_per_page( 'capstone_submissions_per_page', 20 );
	...
	public function render_filter_bar() {
		$current_url = admin_url( 'admin.php?page=curriki-learn-capstone-submissions' );
		...
		echo '<input type="hidden" name="page" value="curriki-learn-capstone-submissions" />';
