        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php if ($teacher_id) : ?>
                    ...
                    <div class="input_section teacher-section">
                        <div class="input_box">
                            <div class="label_box">
                                <label class="label">Assign Teacher</label>
                                <select name="current_teacher_idteacher_id" id="current_teacher_id" disabled class="form-select">
                                    <option value="0">Select Teacher</option>
                                    <?php foreach ($teachers as $teacher) { ?>
                                        <option value="<?php echo $teacher->ID; ?>" <?php echo $teacher_id && $teacher_id == $teacher->ID ? "selected=selected" : ""; ?> ><?php echo $teacher->post_title; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
                ...
                <?php else : ?>
                    <div class="alert alert-warning alert-dismissible fade show" role="alert">
                        Please select <strong>Teacher</strong> to add students.</strong>
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function() {
        ...
        jQuery(takingStudentForm).on('submit', function(e) {
            ...
            $.ajax({
                ...
                jQuery("#take_student_error").html("Please select at least one student!");
                jQuery(".modal_students_loader").html('');
                jQuery("#takenStudentSaveBtn").attr("disabled", false);
            });
    ...
    function getTakenStudents() {
        ...
        var school_id = '<?= isset($_GET['school_id']) ? $_GET['school_id'] : 0 ?>';
        var teacher_id = '<?= isset($_GET['teacher_id']) ? $_GET['teacher_id'] : 0 ?>';
        $.ajax({
            method: "GET",
            url: apiUrl + "students/taken_student",
