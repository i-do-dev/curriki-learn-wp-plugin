        ...
        $(document).ready(function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function(clickEventInfo) {
                    ...
                    $.ajax({
                        url: host + "trek/assigned/students",
                        data: 'event_id=' + clickEventInfo.event.id,
                        type: "POST",
                        async: false,
                        success: function(response) {
                            assignment_students = response;
                        }
                    }).always(function(){
                        let studentProgressModel = '<div>'
                        + '<h4>Student Progress</h4>'
                        + '<hr />'
                        + '<p>'
                        +   '<span id="trekTitleText">'+ jQuery("input#trekTitle").val()+'</span> > <span id="trekSectionText">'+jQuery("input#trekSectionTitle").val()+'</span>'
                        + '</p>'
                        + '<table class="student-progress-table">'
                            + '<tr>'
                                + '<th>Student</th>' + '<th>Status</th>' + '<th>Progress</th>' + '<th>Score</th>'
                            + '</tr>';
                                studentProgressModel += assignment_students.map(student => '<tr>' + '<td>'+ student.name +'</td>' + '<td>'+ student.status +'</td>' + '<td>'+ student.progress +'</td>' + '<td>'+ student.score +'</td>' + '</tr>');
                        studentProgressModel += '</table>'
                        + '</div>';
                        
                        Swal.fire({
                            html: studentProgressModel,
                            buttonsStyling: false,
...
/* 
            var calendar = jQuery('#calendar').fullCalendar({
                ...
                selectable: true,
                selectHelper: true,
                select: function(selectedEvent) {
                    const year = selectedEvent._i[0];
                    const month = selectedEvent._i[1] + 1;
                    const day = selectedEvent._i[2];
                    const hour = selectedEvent._i[3];
                    const minutes = selectedEvent._i[4];
                    const seconds = selectedEvent._i[5];

                    const start = '2023-02-27T15:30:00';
                    const end = '2023-02-27T16:45:00';
                    const allDay = false;
                    console.log("cal selectedEvent ==== ", selectedEvent);
                    return false;
                    $.ajax({
                        url: host + "get/all/treks",
                        type: "get",
    ...
    <input type="hidden" name="trekTitle" id="trekTitle" value="" />
    <input type="hidden" name="trekSectionId" id="trekSectionId" value="" />
    <input type="hidden" name="trekSectionTitle" id="trekSectionTitle" value="" />
    </body>
