...
SceneItem shape:
{
  ...
  "description": "<optional — for framework layout: a JSON array of 2–4 compact phrases, each ~3–4 words (max 5), e.g. [\"Original human synthesis required\", \"No AI shortcuts allowed\", \"Lived reasoning, not generation\"]; for split_blueprint: ONE short phrase (roughly 6–10 words) — narrow item columns; for all other layouts: ONE short sentence (roughly 12–16 words) that fits in 2 lines on screen; NEVER a full paragraph or multiple sentences; required in editorial; useful in card_list and split_blueprint when an item needs more than a label.>"
}
...
| framework          | 3 items — numbered blueprint blocks; each item SHOULD have description as a string[] of 2–4 compact phrases (~3–4 words each, max 5); badge recommended |
...
| cycle_loop         | exactly 4 items — nodes in a diamond-shaped cycle loop with arc arrows; item.text MUST be 1-3 words (a short stage/role name) to fit inside the circular node |
| split_blueprint    | 4-8 items — set role:'input' on left-column items and role:'output' on right-column items; item.text max 4-5 words, sub_label max 4-5 words; if description is used keep it to 6-8 words (columns are narrow, especially with 6-8 items) |
| fuel_engine        | 3-5 items — set role:'input' on ingredients (left), role:'output' on exactly 1 result (right); item.text max 3-5 words; keep each card a SINGLE label line — do NOT add description or sub_label |
| checklist_reveal   | 3-6 items — sequentially revealed checklist; use status:'gap' or 'warn' on weak/missing items; keep each item to ONE concise line of ~6-8 words — do NOT add sub_label or description |
| deployment_circles | exactly 4 items — concentric ring labels (innermost first, e.g. individual → team → org → all); item.text MUST be 1-2 words, single-line |
...
- Description usage: add description when an item needs more than a label. For framework layout, set description to a string[] of 2–4 compact phrases, each ~3–4 words (max 5) — e.g. ["Original human synthesis", "No AI shortcuts", "Lived reasoning required"]. For split_blueprint, keep description to a short phrase (6–10 words) — narrow item columns. Do NOT add description to fuel_engine or checklist_reveal — their items are single label lines. For all other layouts, keep description to ONE short sentence (12–16 words, fits 2 lines) — never a paragraph or multiple sentences. Required in editorial; optional elsewhere. Do not pad with repetition of item.text.
